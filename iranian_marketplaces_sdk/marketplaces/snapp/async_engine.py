...
class SnappAsync:
    """Snapp Shop over the async engine.
    ...
    async def update_products(self, body: list[ProductUpdate]) -> list[ProductUpdateResult]:
    ...
    async def list_orders(self, *, query: VendorOrdersQuery | None = None) -> VendorOrdersResponse:
    ...
    async def get_order(self, order_number: str | int) -> VendorOrderResponse:
        ...
        return parse(
            VendorOrderResponse,
            await self._transport.get(vendor_order_endpoint(self.config.seller_id, order_number)),
        )
