...
from iranian_marketplaces_sdk.marketplaces.digikala.resources import AsyncResources


class DigikalaAsync(AsyncResources):
    """Digikala over the async engine.
    ...
    async def create_token(
        ...
        *,
        # ASYNC109: this is httpx's per-request timeout, not a cancellation deadline — the whole
        # point is that the transport enforces it, so `asyncio.timeout` would be the wrong tool.
        ...
        """
        transport = AsyncTransport(BASE_URL, timeout=timeout, client=client)
        try:
    ...
    async def health_check(self) -> HealthCheckResponse:
        ...
        return parse(HealthCheckResponse, await self._transport.get(HEALTH_CHECK_ENDPOINT))

    async def get_scopes(self) -> ScopesResponse:
        """All scopes defined by the service, not the current token's permissions."""
        return parse(ScopesResponse, await self._transport.get(AUTH_SCOPES_ENDPOINT))
    ...
    async def get_client_scopes(self, client_code: str) -> ScopesResponse:
        ...
        return parse(
            ...
            await self._transport.get(
                auth_client_scopes_endpoint(client_code), headers=JSON_HEADERS
            ),
        )
    ...
    async def get_variant(self, variant_id: int) -> VariantResponse:
        ...
        return parse(
            VariantResponse,
            await self._transport.get(variant_endpoint(variant_id), headers=JSON_HEADERS),
        )
    ...
    async def get_variant_gold(self, variant_id: int) -> VariantGoldResponse:
        ...
        return parse(
            VariantGoldResponse,
            await self._transport.get(variant_gold_endpoint(variant_id), headers=JSON_HEADERS),
        )

    async def get_variant_seller_stock(self, variant_id: int) -> VariantSellerStockResponse:
        """A variant's stock, broken down by where it currently is. Scope: ``variant``."""
        return parse(
            VariantSellerStockResponse,
            await self._transport.get(
                variant_seller_stock_endpoint(variant_id), headers=JSON_HEADERS
            ),
        )
    ...
    async def get_invoice_details(self, invoice_id: int) -> InvoiceDetailResponse:
        ...
        return parse(
            InvoiceDetailResponse,
            await self._transport.get(invoice_details_endpoint(invoice_id), headers=JSON_HEADERS),
        )
