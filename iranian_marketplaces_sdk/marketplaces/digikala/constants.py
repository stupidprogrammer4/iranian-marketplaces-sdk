"""Digikala — base URL, endpoint paths, and the fixed values its API defines."""
...
AUTH_TOKEN_ENDPOINT = "/auth/token"  # noqa: S105 — a URL path, not a credential

# Token refresh (unauthenticated: exchanges an expired access + refresh token).
AUTH_REFRESH_TOKEN_ENDPOINT = "/auth/refresh-token"  # noqa: S105 — a URL path
...
SMART_DISCOUNT_VARIANTS_BATCH_ENDPOINT = f"{SMART_DISCOUNT_VARIANTS_ENDPOINT}/batch"
...
def invoice_items_endpoint(
    ...
    return f"/invoices/{invoice_id}/items/{financial_notation_id}/{calculation_type}"


#: The query-string prefix Digikala's list endpoints use for their filters: ``search[active]=1``.
SEARCH_PARAM_PREFIX = "search"

#: Filter keys whose multiple values Digikala expects underscore-joined (``search[ids]=1_2_3``).
#: Every other multi-value filter is comma-joined: category ids, price terms, statuses, tags.
UNDERSCORE_JOINED_KEYS = frozenset({"ids"})
