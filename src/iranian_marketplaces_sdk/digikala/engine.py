"""Synchronous client for the Digikala Seller Open API."""
...
# Search keys whose multiple values Digikala expects underscore-joined
# (e.g. ``search[ids]=1_2_3``). Every other multi-value filter is
# comma-joined (category ids, price terms, statuses, tags, ...).
_UNDERSCORE_JOINED_KEYS = frozenset({"ids"})


def _format_param_value(key: str, value: object) -> object:
    """Concatenate list/tuple filter values the way Digikala expects.

    A scalar is returned unchanged. A sequence is joined with ``_`` for
    keys in :data:`_UNDERSCORE_JOINED_KEYS`, otherwise with ``,``.
    """
    if isinstance(value, (list, tuple)):
        separator = "_" if key in _UNDERSCORE_JOINED_KEYS else ","
...
def _build_list_params(
    ...
    sort: str | None,
    order: str | None,
    search: Mapping[str, object] | None,
) -> dict[str, object]:
    """Flatten paginated-list arguments into Digikala's query parameters.

    Shared by the variant and order list endpoints. ``search`` keys are
    emitted in the API's bracket form, e.g. ``{"search[active]": True}``.
    Sequence values are concatenated via :func:`_format_param_value`.
    """
    ...
    if search:
        for key, value in search.items():
            params[f"search[{key}]"] = _format_param_value(key, value)
    return params


def _build_flat_list_params(
    page: int | None,
    size: int | None,
    sort: str | None,
    order: str | None,
    ...
    """Like :func:`_build_list_params` but ``filters`` are flat query keys.

    Used by ``GET /orders/history``, whose filters are top-level query
    parameters rather than the ``search[...]`` bracket form.
    """
    params = _build_list_params(page, size, sort, order, None)
    if filters:
        for key, value in filters.items():
            params[key] = _format_param_value(key, value)
    return params


class DigikalaClient(BaseClient):
    """Synchronous API client for Digikala.
    ...
    def create_token(
        ...
        *,
        ...
        """
        bootstrap = BaseClient(BASE_URL, timeout=timeout, client=client)
        try:
            return bootstrap._post(
                AUTH_TOKEN_ENDPOINT,
    ...
    def refresh_token(self) -> TokenResponse:
        ...
        new access token.
        """
        response: TokenResponse = self._post(
            AUTH_REFRESH_TOKEN_ENDPOINT,
            json={
